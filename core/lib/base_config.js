...
class BaseConfig {
  ...
  set(env, validator, defaultVal) {
    ...
    if (process.env[env] || process.env[env] === "") {
      ...
      value = defaultVal;
      warnLogger.warn(
        `Missing env variable: "${env}". Default value was applied: ${defaultVal}`
      );
    }
    ...
    if (
      ...
      if (typeof validator === "object") {
        ...
        throw new Error(
          `Wrong "${env}" variable; Value: "${value}" is invalid. ${joiResult.error}`
        );
      }

      if (!validator(value)) {
        throw new Error(
          `Wrong "${env}" variable; Value: "${value}" is invalid.`
        );
      }
  ...
  setDirect(val, validator, defaultVal) {
    ...
    if (val || val === "") {
      ...
      value = defaultVal;
      warnLogger.warn(
        `Missing direct value: "${val}". Default value was applied: ${defaultVal}`
      );
    }
