...
class BaseModel {
  constructor(src) {
    ...
    Object.keys(schema).forEach((propName) => {
      ...
      if (!Validator.isBoolean(validationResult)) {
        throw new Error(
          `Invalid '${propName}' field validator. Please redefine it. Validator should return only boolean type.`
        );
      }
      ...
      if (!validationResult) {
        throw new AppError({
          ...
          message: `Invalid '${propName}' field. Expect: ${description}`,
        });
      }
    });
