...
class Logger extends AbstractLogger {
  constructor({ appName, capture = false, sentryDsn, raw = false } = {}) {
    ...
    if (capture && !sentryDsn) {
      throw new Error(
        `${this.constructor.name}: Please define 'sentryDsn' param`
      );
    }
  ...
  _captureException(error, payload) {
    if (this[$].sentryCatch)
      this[$].sentryCatch.captureException(error, payload);
  }

  _captureMessage(message, payload) {
    if (this[$].sentryCatch)
      this[$].sentryCatch.captureMessage(message, payload);
  }
  /**
   * ------------------------------
   * @ERROR_METHODS
  ...
  fatal(message, error, meta) {
    ...
    const payload = validator.isObject(meta)
      ? { ...error, ...meta }
      : { ...error, meta };
